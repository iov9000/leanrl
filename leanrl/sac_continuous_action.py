    ...
    def evaluate_policy(n_episodes: int) -> float:
        ...
        with torch.no_grad():
            while len(ep_returns) < n_episodes:
                mean_action = actor.get_action(torch.as_tensor(obs, device=device, dtype=torch.float32))[2]
            ...
            if global_step % 100 == 0 and start_time is not None:
                ...
                with torch.no_grad():
                    ...
                    if 'actor_loss' in locals():
                        logs["actor_loss"] = actor_loss.mean()
                    if 'alpha_loss' in locals():
                        logs["alpha_loss"] = alpha_loss.mean()
                    if 'qf_loss' in locals():
                        logs["qf_loss"] = qf_loss.mean()
                wandb.log(
                    {
                        "speed": speed,
                        **logs,
