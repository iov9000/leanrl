...
class IdentityPhi(nn.Module):
    """Identity feature map phi(s,a) = concat(s,a)."""
    def forward(self, obs: torch.Tensor, act: torch.Tensor) -> torch.Tensor:
...
class Args:
    ...
    # No Successor Features (ablation)
    no_sf: bool = False

    # Phi Mode: 'learned' | 'id' | 'random_encoder'
    sf_phi_mode: str = "learned"

    # SF Training Improvements
    sf_use_recent_buffer: bool = False
    sf_recent_buffer_size: int = 20_000
    sf_mix_expert_data: bool = False
    sf_expert_fraction: float = 0.5
...
if __name__ == "__main__":
    ...
    # Successor feature models
    if args.sf_phi_mode == "id":
        # phi is identity; psi maps (obs, act) -> (obs, act) dimension
        d_embed = n_obs + n_act
        phi = IdentityPhi()
        # Ensure psi output matches phi output
        psi = PsiNet(n_obs, n_act, d=d_embed, device=device)
        psi_target = PsiNet(n_obs, n_act, d=d_embed, device=device)
        psi_target.load_state_dict(psi.state_dict())
        # Optimizing only psi, since phi has no params
        sf_optimizer = optim.Adam(psi.parameters(), lr=args.sf_lr, capturable=args.cudagraphs and not args.compile)
    elif args.sf_phi_mode == "random_encoder":
        # phi is a random network, not trained; psi maps to sf_dim
        ...
        psi_target.load_state_dict(psi.state_dict())
        # Optimizing only psi; phi is frozen
        sf_optimizer = optim.Adam(psi.parameters(), lr=args.sf_lr, capturable=args.cudagraphs and not args.compile)
    elif args.sf_phi_mode == "learned":
        # Standard: learn both phi and psi mapping to latent dimension sf_dim
        phi = PhiNet(n_obs, n_act, d=args.sf_dim, device=device)
        psi = PsiNet(n_obs, n_act, d=args.sf_dim, device=device)
        psi_target = PsiNet(n_obs, n_act, d=args.sf_dim, device=device)
        psi_target.load_state_dict(psi.state_dict())
        ...
        raise ValueError(f"Unknown --sf_phi_mode {args.sf_phi_mode}")

    # Replay
    envs.single_observation_space.dtype = np.float32
    rb = ReplayBuffer(storage=LazyTensorStorage(args.buffer_size, device=device))

    # Optional small recent buffer for SF to reduce off-policy drift
    sf_rb = None
    if args.sf_use_recent_buffer:
        sf_rb = ReplayBuffer(storage=LazyTensorStorage(args.sf_recent_buffer_size, device=device))

    # Expert Data for SF mixing
    expert_rb = None
    if args.sf_mix_expert_data:
        expert_next_obs = _build_next_obs_from_obs_done(expert_obs, demos_all.get("done", None))
        ...
        expert_td = TensorDict(
            ...
            actions=expert_acs,
            rewards=torch.zeros((expert_N, 1), device=device), # dummy rewards
            dones=(demos_all["done"].view(-1, 1) > 0.5) if "done" in demos_all else torch.zeros((expert_N, 1), dtype=torch.bool, device=device),
            terminations=(demos_all["done"].view(-1, 1) > 0.5) if "done" in demos_all else torch.zeros((expert_N, 1), dtype=torch.bool, device=device),
            truncations=torch.zeros((expert_N, 1), dtype=torch.bool, device=device),
            batch_size=[expert_N],
            device=device,
        )
    ...
    def update_sf(data: TensorDict):
        ...
        dones = (data["terminations"] | data["truncations"]).float().view(-1, 1)

        with torch.no_grad():
            next_act, _, _ = actor.get_action(next_obs)
            psi_next = psi_target(next_obs, next_act)
            target = phi(obs, act) + (1.0 - dones) * args.sf_gamma * psi_next
    ...
    def compute_ot_rewards(data: TensorDict):
        ...
        # Use phi (instantaneous) if no_sf is True, else psi (successor features)
        if args.no_sf:
            z_pi = F.normalize(phi(obs, act), dim=-1)
            z_e = F.normalize(phi(e_obs, e_act), dim=-1)
        else:
            z_pi = F.normalize(psi(obs, act), dim=-1)
            z_e = F.normalize(psi(e_obs, e_act), dim=-1)
    ...
    def update_main(data: TensorDict):
        ...
        with torch.no_grad():
            ...
            if "rewards_override" in data.keys():
                rewards = data["rewards_override"]
    ...
    for global_step in pbar:
        ...
        transition = TensorDict(
            ...
            truncations=torch.as_tensor(truncations, device=device).view(-1, 1),
        ...
        if global_step > args.learning_starts:
            ...
            if (global_step % args.sf_update_frequency) == 0:
                # Determine source for SF data
                if sf_rb is not None:
                    # Sample from recent buffer (on-policy approximation)
                    sf_data = sf_rb.sample(args.batch_size)
                else:
                    # Sample from main replay buffer
                    sf_data = rb.sample(args.batch_size)
                
                # Mix expert data if requested
                if args.sf_mix_expert_data and expert_rb is not None:
                    n_exp = int(args.batch_size * args.sf_expert_fraction)
                    n_pol = args.batch_size - n_exp
                    exp_batch = expert_rb.sample(n_exp)
                    
                    if n_pol > 0:
                        pol_batch = sf_data if sf_data.shape[0] == n_pol else sf_rb.sample(n_pol) if sf_rb else rb.sample(n_pol)
                        # Concatenate
                        sf_data = TensorDict(
                            {
                                "observations": torch.cat([exp_batch["observations"], pol_batch["observations"]], dim=0),
                                "next_observations": torch.cat([exp_batch["next_observations"], pol_batch["next_observations"]], dim=0),
                                "actions": torch.cat([exp_batch["actions"], pol_batch["actions"]], dim=0),
                                "dones": torch.cat([exp_batch["dones"], pol_batch["dones"]], dim=0),
                                "terminations": torch.cat([exp_batch["terminations"], pol_batch["terminations"]], dim=0),
                                "truncations": torch.cat([exp_batch["truncations"], pol_batch["truncations"]], dim=0),
                            },
            ...
            if rewards_override is not None:
                data["rewards_override"] = rewards_override
